"""
Q&A service — simple, replaceable context assembly + streaming + persistence.

Context assembly is isolated in assemble_context() so the knowledge phase
can swap it out without touching streaming or persistence logic.
...
# Truncation limits to keep context tight
_MAX_SELECTED_CHARS = 2_000
_MAX_CHUNK_CHARS = 400


# ---------------------------------------------------------------------------
# Context assembly — the one function that will be replaced in the knowledge phase
# ---------------------------------------------------------------------------
...
def assemble_context(
    ...
    question: str,
    chunks: list[str],
    recent_turns: list[tuple[str, str]],  # [(user_q, assistant_a), ...]
) -> str:
    parts: list[str] = []
...
async def _get_recent_turns(
    ...
    """Return last n_pairs Q&A pairs in chronological order."""
    result = await db.execute(
        select(Message)
...
async def stream_qa(
    ...
    try:
        async for delta in provider.stream_text(messages, max_tokens=1024):
